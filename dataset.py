...
class Datasets(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 读取原始图片和标签，并转RGB
        img_o = cv2.imread(os.path.join(img_path[0], name1))    # 读取tif 和 jpg 类型
        _, img_l = cv2.VideoCapture(os.path.join(img_path[1], name2)).read()   # 读取 gif 和 tif 类型
...
if __name__ == '__main__':
    i = 1
    dataset = Datasets(r".\DRIVE\training2")
    for a, b in dataset:
        print(i)
        # print(a.shape)
        # print(b.shape)
